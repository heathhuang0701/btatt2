...
class ControllerProductSearch extends Controller {
	public function index() {
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			if($page>1) $data['pagination']['last'] = $this->url->link('product/search',$url . '&page=' .(int)($page-1));
			for($i=($page-2);$i<=($page+2);$i++)
			{
				if($i > 0 && $i <= $all_page) 
				{
					$data['pagination']['num'][$i] = $this->url->link('product/search',$url . '&page=' .(int)$i);
					$data['pgcnt']--;
				}
			}
			if($page < $all_page) $data['pagination']['next'] = $this->url->link('product/search',$url . '&page=' .(int)($page+1));

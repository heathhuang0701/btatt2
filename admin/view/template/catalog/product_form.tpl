      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          <ul class="nav nav-tabs">
			...
			<li><a href="#tab-add-price" data-toggle="tab"><?php echo $tab_add_price; ?></a></li>
          ...
          <div class="tab-content">
            ...
            <div class="tab-pane" id="tab-links">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
				  <select id="cate_select" multiple="true" list="schList" listKey="schId" listValue="schNm" style="width:80%;height: 150px;">
                    <?php foreach ($all_categories as $all_category) { ?>
                    <option id="product-category<?php echo $all_category['category_id']; ?>" value="<?php echo $all_category['category_id']; ?>"><?php echo $all_category['name']; ?></option>
                    <?php } ?>
				  </select>

				<input type="button" class="btn" value="加入" onclick="add_cate_sel();" ></input>
			...
			<div class="tab-pane" id="tab-add-price">
			  <div class="table-responsive">
                <table id="add_price" class="table table-striped table-bordered table-hover">
                  <thead>
                    <tr>
                      <td class="text-left"><?php echo $entry_add_id; ?></td>
                      <td class="text-left"><?php echo $entry_add_name; ?></td>
					  <td class="text-left"><?php echo $entry_add_price; ?></td>
                      <td></td>
                    </tr>
                  </thead>
                  <tbody>
                    <?php $add_row = 0; ?>
                    <?php foreach ($add_prices as $add_price) { ?>
                    <tr id="add-row<?php echo $add_row; ?>">
                      <td class="text-left" style="width: 20%;">
						<?php echo $add_price['product_id']; ?>
                        <input type="hidden" class="add_ids" name="add_price[<?php echo $add_row; ?>][product_id]" value="<?php echo $add_price['product_id']; ?>" />
					  </td>
					  <td class="text-left" style="width: 50%;">
					    <?php echo $add_price['name']; ?>
						<input type="hidden" name="add_price[<?php echo $add_row; ?>][name]" value="<?php echo $add_price['name']; ?>" />
					  </td>
                      <td class="text-left">
                        <input type="text" name="add_price[<?php echo $add_row; ?>][add_price]" value="<?php echo $add_price['add_price']; ?>" placeholder="<?php echo $entry_add_price; ?>" class="form-control" />
					  </td>
                  ...
                  <tfoot>
                    <tr>
                      <td colspan="4">
						<input type="text" id="fillin" value="" placeholder="<?php echo $entry_product_name; ?>" class="form-control" />
					  </td>
                    </tr>
                  </tfoot>
                </table>
...
$('#fillin').autocomplete({
	...
	{
		var none = 1;
		$('.add_ids').each(function(){
			if( parseInt(item['value']) == parseInt($(this).val()) )
			{
				none = 0;
			}
		...
		if(parseInt(item['value']) == parseInt('<?php echo $id; ?>'))
		{
			none = 0;
		}
		...
		if(none == 1)
		{
			...
			html += item['value'];
			html += '<input type="hidden" class="add_ids" name="add_price[' + add_row + '][product_id]" value="' + item['value'] + '" />';
			html += '</td>';			  
			html += '<td class="text-left" style="width: 50%;">';
			html += item['label'];
			html += '<input type="hidden" name="add_price[' + add_row + '][name]" value="' + item['label'] + '" />';
			html += '</td>';
			html += '<td class="text-left">';
			html += '<input type="text" name="add_price['+add_row+'][add_price]" value="" placeholder="<?php echo $entry_add_price; ?>" class="form-control" />';
			html += '</td>';
...
function add_cate_sel()
{
	var a = $('#cate_select').val();
	if(a == null)
	{
	}
	else
	{
		var b = $('#product-category'+a).html();
		
		$('#product-category').append('<div id="product-category' + a + '"><i class="fa fa-minus-circle"></i> ' + b + '<input type="hidden" name="product_category[]" value="' + a + '" /></div>');	
	}
}

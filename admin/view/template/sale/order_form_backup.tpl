...
$('#button-customer').on('click', function() {
	var a = $('#tab-customer input[type=\'text\'], #tab-customer input[type=\'hidden\'], #tab-customer input[type=\'radio\']:checked, #tab-customer input[type=\'checkbox\']:checked, #tab-customer select, #tab-customer textarea');
	...
	$.ajax({
		...
		error: function(xhr, ajaxOptions, thrownError) {
			//alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
			console.log('index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/customer&store_id=0' );

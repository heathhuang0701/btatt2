...
class ControllerSaleCustomer extends Controller {
	...
	public function unlock() {
		...
		if (isset($this->request->get['account']) && $this->validateUnlock()) {
			$this->model_sale_customer->deleteLoginAttempts($this->request->get['account']);
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$login_info = $this->model_sale_customer->getTotalLoginAttempts($result['account']);

			if ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) {
				$unlock = $this->url->link('sale/customer/unlock', 'token=' . $this->session->data['token'] . '&account=' . $result['account'] . $url, 'SSL');
			} else {
				$unlock = '';
			}
		...
		$data['sort_account'] = $this->url->link('sale/customer', 'token=' . $this->session->data['token'] . '&sort=c.account' . $url, 'SSL');
		$data['sort_name'] = $this->url->link('sale/customer', 'token=' . $this->session->data['token'] . '&sort=c.name' . $url, 'SSL');
	...
	protected function getForm() {
		...
		if (isset($this->request->post['account'])) {
			...
			$data['account'] = $customer_info['account'];
		} else {
			$data['account'] = '';
		}
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['account']) < 8) || (utf8_strlen(trim($this->request->post['account'])) > 20)) {
		...
		$customer_info = $this->model_sale_customer->getCustomerByAccount($this->request->post['account']);
		...
		if (isset($this->request->post['address'])) {
			foreach ($this->request->post['address'] as $key => $value) {
				if ((utf8_strlen($value['name']) > 32)) {
					$this->error['address'][$key]['name'] = $this->language->get('error_name');
				}

				if ((utf8_strlen($value['address']) > 128)) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email']) || isset($this->request->get['filter_account'])) {
			if (isset($this->request->get['filter_name'])) {
				$filter_name = $this->request->get['filter_name'];
			} else {
				$filter_name = '';

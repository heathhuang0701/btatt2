...
class ControllerUserUserPermission extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) 
		{
			...
			#every user has the right to view
			$all_yes = array(
				'common/column_left',
				'common/menu',		
				'common/profile',
			...
			foreach($all_yes as $rows)
			{
				$this->request->post['permission']['access'][] = $rows;
				$this->request->post['permission']['modify'][] = $rows;
			}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) 
		{
			...
			#extra add bases for super_admin
			$adonly = array(
				'dashboard/activity',
				'dashboard/chart',
				'dashboard/customer',
			...
			if($this->request->get['user_group_id'] == '0' && $this->user->super_admin == '1')
			{
				foreach($adonly as $rows)
				{
					$this->request->post['permission']['access'][] = $rows;
					$this->request->post['permission']['modify'][] = $rows;
				}
	...
	protected function getForm() {
		...
		{
			$check_auth = $this->model_user_user_group->getGroupPermission('0');
		}
		else
		{
			$check_auth = $this->model_user_user_group->getGroupPermission('1');
		}
		foreach ($files as $file) 
		{
			...
			{
				if(in_array($permission, $check_auth['access']) || $this->user->super_admin == '1')
				{
					$data['permissions']['access'][] = $permission;
					$data['permissions_text']['access'][] = $this->language->get($permission);
				}
				
				if(in_array($permission, $check_auth['modify']) || $this->user->super_admin == '1')
				{
					$data['permissions']['modify'][] = $permission;
					$data['permissions_text']['modify'][] = $this->language->get($permission);
				}
			}
		}

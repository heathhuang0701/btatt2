...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		$data['entry_add_id'] = $this->language->get('entry_add_id');
		$data['entry_add_name'] = $this->language->get('entry_add_name');
		$data['entry_add_price'] = $this->language->get('entry_add_price');	
		...
		$data['button_new_add'] = $this->language->get('button_new_add');
		...
		$data['tab_add_price'] = $this->language->get('tab_add_price');
		...
		$all_cate = $this->model_catalog_category->getCategories($cate_filter);

		foreach($all_cate as $cates) 
		{	
			$data['all_categories'][] = array(
				'category_id' => $cates['category_id'],
				'name'        => strip_tags(html_entity_decode($cates['name'], ENT_QUOTES, 'UTF-8'))
			);
		}
		...
		// Add Price
		$this->load->model('catalog/add_price');
		
		if(isset($this->request->post['add_price']))
		{
			$add_prices = $this->request->post['add_price'];
		}
		elseif(isset($this->request->get['product_id']))
		{
			$add_prices = $this->model_catalog_add_price->getItem($this->request->get['product_id']);
		}
		else
		{
			$add_prices = array();
		...
		$data['add_prices'] = array();

		foreach($add_prices as $add_price) 
		{
			$data['add_prices'][] = array(
				'name' => $add_price['name'],
				'product_id' => $add_price['product_id'],
				'add_price' => $add_price['add_price']
			);
		}
	...
	protected function validateForm() {
		...
		foreach($this->request->post['add_price'] as $add_id => $add_price)
		{
			if(!is_numeric($add_price['product_id'])) 
			{
				$this->error['warning'] = $this->language->get('error_add_price');
			}
			
			if(!is_numeric($add_price['add_price']) || $add_price['add_price'] == '') 
			{
				$this->error['warning'] = $this->language->get('error_add_price');
			}
		}

<?php
class ModelApiReward extends Model {
				...
				foreach ($this->cart->getProducts() as $product)
				{
					...
					if(count($product['add_products']) > 0)
					{
						foreach($product['add_products'] as $add)
						{
							if($add['points'])
							{
								$discount = $add['total'] * ($this->session->data['reward'] / $points_total);
		
								if ($add['tax_class_id']) {
									$tax_rates = $this->tax->getRates($add['total'] - ($add['total'] - $discount), $add['tax_class_id']);

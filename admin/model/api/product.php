...
class ModelApiProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_ids'])) {
				...
				$sql .= " AND p.product_id IN (" . implode(',', $data['filter_ids']) . ")";
		}
		
		if (!empty($data['filter_txt'])) {

				$sql .= " AND p.product_id IN (" .$data['filter_txt'] . ")";
		}
	...
	public function getProductAddItems($product_id)
	{
		$sql = "SELECT * FROM " . DB_PREFIX . "add_price a 
				LEFT JOIN " . DB_PREFIX . "product p ON (a.add_product_id = p.product_id) 
				LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (a.add_product_id = p2s.product_id) 
	...
	public function getProductAddItemByMultiple($product_id,$items=array())
	{
		$counts = 0;
		$def = '';
		if($items)
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "add_price a 
				...
				LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (a.add_product_id = p2s.product_id) 
				LEFT JOIN " . DB_PREFIX . "product_description pd ON (a.add_product_id = pd.product_id) 
				WHERE a.product_id = '" . (int)$product_id . "' AND p.status = '1' AND a.add_product_id IN (".$def.")
				AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'
				AND pd.language_id = ".(int)$this->config->get('config_language_id')." ";
